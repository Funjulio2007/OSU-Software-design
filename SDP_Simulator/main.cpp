    ...
    void start () // this is in its sample mode. Need to add stuff 
{
    ...
    int i = 0, direction = 1, enemy_hit = 0;
...
time_move = TimeNow();
    ...
    else if (Keyboard.isPressed (KEY_SPACE) && shoot_cooldown > .15) 
    {
...
void endscreen()
{
    ...
    LCD.Clear();
    FEHImage skull;
    skull.Open ("backgroundscreenskull.png");
    skull.Draw(30,50);
    LCD.SetFontColor(WHITE);
    LCD.SetFontScale(1);
