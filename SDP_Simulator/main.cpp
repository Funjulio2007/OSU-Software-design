...
class collectstats 
{
    private:
    int score, waves;
    float Time_alive; // I have to collect these values from the start function
    char playername [4];
    public:
    void STATS();
    void endscreen();
    void start();
    collectstats(char name[]= "None");


};

//constructor
collectstats::collectstats(char name [4])
{
...
time_move = TimeNow();
    ...
    else if (lanternflyy > 180) //checks if the lanternflies have reached the bottom of the screen
        {
...
wave_over = 1; //breaks out of this loop, but quit is still set to 0, so a new wave will start
    ...
    Sleep (5); //slow down the loop
        background_refresh (rocketx, rockety, lanternflyx, lanternflyy, enemy_hit); //run background refresh to regenerate the environment with new positions
...
{
        /*Wave transition!*/
background_refresh (rocketx, rockety, lanternflyx, lanternflyy, enemy_hit); //call background refresh once to make sure that all elements of the background are updated
...
move_cooldown = move_cooldown - 0.015; //make the new wave move faster than the last
lanternflyx = 120;
lanternflyy = 0; //reset the lanternfly position
background_refresh (rocketx, rockety, lanternflyx, lanternflyy, enemy_hit); //get rid of the old background and re-generate the enemies for the next wave
